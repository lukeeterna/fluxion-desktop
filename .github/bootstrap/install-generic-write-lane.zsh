...
elif [ "$ROUTER_SHA" = "$EXPECTED_ROUTER_SRC_SHA" ] && [ "$GEN_BLOB" = "$EXPECTED_GENERIC_SRC_BLOB" ] && [ "$MARKER_STATE" = "present" ]; then
...
[ "$GEN_BLOB" = "$EXPECTED_GENERIC_SRC_BLOB" ] || { echo "STOP: installed generic blob mismatch"; exit 2; }
...
echo "GENERIC_WRITE_BLOB=$GEN_BLOB"
