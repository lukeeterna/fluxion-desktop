...
- verticali system: ESISTE_NON_TESTATO — **INCOERENZA RISOLTA S320 (code-truth)**: canonico = `src/types/setup.ts:66` `MACRO_CATEGORIE` = **8 macro** (medico, beauty, hair, auto, wellness, professionale, pet, formazione) + `MICRO_CATEGORIE:129` ~50 micro con `hasScheda`. L'array "5 valori" che l'audit S308 citava a :239 è `CATEGORIE_ATTIVITA` sotto commento `CONSTANTS LEGACY:230` = morto/non canonico. Schede React + DB reali per **6/8 macro** (medico, beauty, hair, auto, wellness, pet); `professionale`+`formazione` = gusci vuoti. switch_vertical.sh 9 verticali = istanze test concrete (DB demo `create_vertical_dbs.py`) mappate nei 6 macro implementati = matrice test Sara live. E2E=NO.
...
2. **Verticali canonici — RISOLTO S320 (code-truth)**: fonte di verità = `src/types/setup.ts:66` `MACRO_CATEGORIE` = **8 macro** (medico, beauty, hair, auto, wellness, professionale, pet, formazione) + `MICRO_CATEGORIE:129` ~50 micro con flag `hasScheda`. NB: `CATEGORIE_ATTIVITA:238` (5 valori salone/auto/wellness/medical/altro) è marcato `CONSTANTS LEGACY:230` = morto, NON canonico (errore audit S308). Schede React + DB realmente implementate per **6 macro su 8** (medico, beauty, hair, auto, wellness, pet); `professionale` + `formazione` = gusci vuoti (nessuna scheda, nessun DB demo Sara). **Matrice test Sara (punto 1)** = 9 verticali voice-agent con DB demo pronti (`create_vertical_dbs.py`): salone, barbiere, beauty, odontoiatra, fisioterapia, gommista, toelettatura, palestra, medical → coprono i 6 macro implementati.
