#!/usr/bin/env bash
# launch_rig.sh — RIG high-port per B3-X2-PROVE (congedo M5 su rig multi-inject)
# Ricostruito da: REPORT_RIG-A3_2026-07-10-f.md (§56-64) + rig/D3-DONE_20260710.md (§21).
...
# Eseguito SU iMac (via: ssh imac 'bash -s' < launch_rig.sh).
set -uo pipefail
...
guard() {
  ...
  # Porte protette: confronto ESATTO per-porta (evita match-substring 15062⊃5062, 15090⊃5090).
  ...
  # SIP server: deve essere loopback e la sua porta non deve essere 3002/5062/5090.
  ...
  # Nessun riferimento al trunk EHIWEB/DID.
...
for i in $(seq 1 25); do
  ...
  st_ns="${st// /}"   # tollera JSON con spazi ("registered": true)
  case "$st_ns" in
