...
prompt. All model prompts are fixed, versioned PUBLIC strings generated here.

The live Codex worker is never selected directly by the harness: every stage first
passes through the repo-certified Fabric router, which is pinned by exact SHA and
fails closed on cost, data-policy, registry, or quota state.
"""
...
FABRIC_CERTIFICATION_SHA = "18752fb147f6700d4dc0633a55b9367d00ef0d91"
CODEX_WORKER_ID = "codex-g2-live"
DATA_CLASS = "PUBLIC"
DATA_POLICY = "PUBLIC_ONLY"
DATA_POLICY_REF = "T-VOS-FABRIC-G4-G5#PUBLIC_ONLY"
...
def load_fabric(fabric_dir: Path):
    ...
    return bridge, checkpoint, rollover, router, worker_adapter
...
def validate_g2_qualification_ref(value: str) -> str:
    ref = value.strip()
    if not ref:
        raise RuntimeError("G2 qualification reference is required")
    required_fragments = (
        "G2=GREEN",
        "run=",
        "job=",
        "fabric=%s" % FABRIC_CERTIFICATION_SHA,
    )
    for fragment in required_fragments:
        if fragment not in ref:
            raise RuntimeError("G2 qualification reference missing %s" % fragment)
    return ref


def build_worker_registry(model: str, g2_qualification_ref: str):
    ref = validate_g2_qualification_ref(g2_qualification_ref)
    return [
        {
            ...
            "enabled": True,
            "priority": {DATA_CLASS: 0},
        }
    ]


def select_worker(router, model: str, g2_qualification_ref: str):
    registry = build_worker_registry(model, g2_qualification_ref)
    request = {
        "data_class": DATA_CLASS,
        "max_cost_usd": 0,
        "eligible_workers": [CODEX_WORKER_ID],
    }
    decision = router.route_request(
        request,
        registry,
        {CODEX_WORKER_ID: router.default_breaker()},
    )
    if decision.get("status") != "ROUTE":
        raise RuntimeError("Fabric router blocked G4/G5: %s" % decision.get("reason"))
    worker = decision.get("worker")
    if not isinstance(worker, Mapping):
        raise RuntimeError("Fabric router returned no worker")
    if worker.get("worker_id") != CODEX_WORKER_ID:
        raise RuntimeError("Fabric router selected unexpected worker")
    if worker.get("kind") != "DIRECT_OFFICIAL":
        raise RuntimeError("Codex worker must remain DIRECT_OFFICIAL")
    if worker.get("data_policy") != DATA_POLICY or worker.get("data_classes") != [DATA_CLASS]:
        raise RuntimeError("Codex worker must remain PUBLIC_ONLY for this certification")
    if worker.get("qualification_ref") != g2_qualification_ref:
        raise RuntimeError("router worker is not bound to exact G2 qualification reference")
    if worker.get("max_cost_usd") != 0 or worker.get("zero_cost_hard_stop") is not True:
        raise RuntimeError("router worker violates zero-cost hard stop")
    if worker.get("consumer_oauth_proxy") is not False:
        raise RuntimeError("consumer OAuth proxying is forbidden")
    return dict(worker), canonical_json_sha256(decision)
...
def make_worker_spec(
    ...
    *,
    ...
    if selected_worker.get("model_id") != model:
        raise RuntimeError("router/model mismatch")
    if selected_worker.get("data_policy") != DATA_POLICY:
        raise RuntimeError("router/data-policy mismatch")
...
def main() -> int:
    ...
    parser.add_argument("--g2-qualification-ref", required=True)
    parser.add_argument("--g2-source-thread-id")
    args = parser.parse_args()

    if args.fabric_sha != FABRIC_CERTIFICATION_SHA:
        raise RuntimeError("runtime Fabric SHA does not match routed certification SHA")
    g2_qualification_ref = validate_g2_qualification_ref(args.g2_qualification_ref)
    ...
    bridge, checkpoint, rollover, router, worker_adapter = load_fabric(fabric_dir)
    selected_worker, router_decision_sha = select_worker(
        router,
        args.model,
    ...
    if args.stage == "stage1":
        ...
        write_json(
            stage1_path,
            {
                ...
                "data_class": DATA_CLASS,
                "data_policy": DATA_POLICY,
                "router_decision_sha256": router_decision_sha,
                "g2_qualification_ref_sha256": sha256_text(g2_qualification_ref),
                "context_used_units": units,
                "context_limit_units": context_limit,
    ...
    if stage1.get("router_decision_sha256") != router_decision_sha:
        raise RuntimeError("router decision changed across process boundary")
    if stage1.get("g2_qualification_ref_sha256") != sha256_text(g2_qualification_ref):
        raise RuntimeError("G2 qualification reference changed across process boundary")
    ...
    final_evidence = {
        ...
        "g5": "GREEN",
        "router": "GREEN",
        "router_worker_id": CODEX_WORKER_ID,
        "router_decision_sha256": router_decision_sha,
        "g2_qualification_ref_sha256": sha256_text(g2_qualification_ref),
        "data_class": DATA_CLASS,
        "data_policy": DATA_POLICY,
