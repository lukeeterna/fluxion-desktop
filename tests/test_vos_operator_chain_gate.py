...
SPEC = importlib.util.spec_from_file_location(
    "vos_operator_chain_gate",
    HERE / "bin/vos_operator_chain_gate.py",
)
...
class OperatorChainGateTests(unittest.TestCase):
    ...
    def test_blocks_action_reviewer_substitution(self):
        temporary, root = self.copy_fixture()
        self.addCleanup(temporary.cleanup)
        (root / ".github/workflows/fluxion-sonnet-review.yml").write_text(
            "name: forbidden\n"
        )
        result = gate.validate(root)
    ...
    def test_reject_operator_capability_reassignment(self):
        temporary, root = self.copy_fixture()
        self.addCleanup(temporary.cleanup)
        path = root / "docs/judge/OPERATOR-CHAIN.json"
    ...
    def test_reject_operator_identity_reassignment(self):
        ...
        path = root / "docs/judge/OPERATOR-CHAIN.json"
        data = json.loads(path.read_text())
        data["operators"][2]["identity"] = "Claude Code Action"
        path.write_text(json.dumps(data))
        with self.assertRaises(gate.GateError):
            gate.validate(root)
