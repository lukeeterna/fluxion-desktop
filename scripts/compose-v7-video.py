#!/usr/bin/env python3
"""
FLUXION V7 — Final Video Compositing
41 scenes with animated number overlays, music mixing, logo watermark.
Reads V7 storyboard + v7-voiceover-manifest.json.
Output: landing/assets/fluxion-promo-v7.mp4
"""
...
OUTPUT = BASE / "landing" / "assets" / "fluxion-promo-v7.mp4"
TMPDIR = BASE / "tmp-video-build"
MANIFEST = TMPDIR / "v7-voiceover-manifest.json"
...
SCENES = [
    ...
    # CH3: Sara + dialogo (V16/V17 CROPPED senza bordi pellicola)
    ...
    # CH6: Media upload / Portfolio
    {"id": "s30", "type": "ai", "file": "V15_foto_portfolio.mp4", "trim": (0, 5), "transition": "crossfade"},
    {"id": "s31", "type": "screenshot", "file": "21-trasformazioni-prima-dopo.png", "transition": "crossfade"},
...
def build_scene_clip(i, scene, manifest):
    ...
    clip_out = TMPDIR / f"v7_clip_{i:02d}_{sid}.mp4"
    ...
    if stype == "endcard":
        ...
        if numbers and clip_out.exists():
            overlay_path = TMPDIR / f"v7_overlay_{sid}.mov"
            create_number_overlay_video(numbers, target_dur, overlay_path)
            if overlay_path.exists():
                final_clip = TMPDIR / f"v7_clip_{i:02d}_{sid}_num.mp4"
                run_ff([
                    "ffmpeg", "-y",
                    "-i", str(clip_out),
                    "-i", str(overlay_path),
        ...
        prepared = TMPDIR / f"v7_prepared_{sid}.png"
...
def main():
    ...
    print("=" * 60)
    print("  FLUXION V7 — Final Video Compositing")
    ...
    for ci, cp in enumerate(clip_paths):
        norm_path = TMPDIR / f"v7_norm_{ci:02d}.ts"
        run_ff([
            "ffmpeg", "-y", "-i", cp,
    ...
    concat_file = TMPDIR / "v7_concat.txt"
    with open(concat_file, "w") as f:
        for np in norm_paths:
            f.write(f"file '{np}'\n")

    concat_out = TMPDIR / "v7_concatenated.mp4"
    run_ff([
    ...
    music_out = TMPDIR / "v7_with_music.mp4"
    ...
    print("\n" + "=" * 60)
    print(f"  FLUXION V7 — VIDEO COMPLETATO")
