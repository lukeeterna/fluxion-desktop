...
def test_after_constraint_display(text):
    ...
    assert display.startswith("dopo le"), (
        f"Expected 'dopo le ...' got '{display}' for '{text}'"
    )
    assert "alle" not in display, (
        f"Display should NOT contain 'alle' for AFTER constraint: '{display}'"
    )
...
def test_after_constraint_not_exact(text):
    ...
    assert result.time_constraint.constraint_type != TimeConstraintType.EXACT, (
        "'dopo le X' non deve essere EXACT — root cause del bug originale"
    )
...
def test_before_constraint_display(text):
    ...
    assert result is not None
    display = result.time_constraint.display()
    assert display.startswith("prima delle"), (
        f"Expected 'prima delle ...' got '{display}'"
    )
...
@pytest.mark.parametrize(
    "text,expected_type,expected_anchor",
    [
...
def test_matches_range():
    tc = TimeConstraint(
        TimeConstraintType.RANGE, range_start=time(14, 0), range_end=time(16, 0)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            TimeConstraint(TimeConstraintType.BEFORE, anchor_time=time(14, 0)),
            "prima delle",
        ),
        (
            TimeConstraint(TimeConstraintType.AROUND, anchor_time=time(15, 0)),
            "verso le",
        ),
...
def test_constraint_type_roundtrip():
    ...
    for ct in TimeConstraintType:
        assert TimeConstraintType(ct.value) == ct, f"Roundtrip failed for {ct}"
