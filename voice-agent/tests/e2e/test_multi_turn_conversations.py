...
def process(text: str, session_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if VERBOSE:
        print(
            f"  [process] '{text}' → state={response.get('fsm_state')} intent={response.get('intent')} ms={response['_latency_ms']:.0f}"
        )
...
def assert_response_contains(
    response_text: str, *keywords: str, must_not_contain: List[str] = None
) -> bool:
...
class TestHappyPath:
    ...
    def test_returning_client_complete_booking(self):
        ...
        assert assert_response_contains(
            t4["response"], "conferma", "ok", "riepilog", "giusto", "corretto"
        ...
        assert assert_response_contains(
            t5["response"], "confermata", "prenotazione", "whatsapp"
...
class TestAmbiguousName:
    ...
    def test_ambiguous_name_disambiguation(self):
        ...
        if t2["fsm_state"] in ("waiting_surname", "disambiguating_name"):
            assert (
                "cognome" in response_lower
                or "quale" in response_lower
                or "marco" in response_lower
        ...
        if "cognome" in response_lower or "surname" in response_lower:
            ...
            assert t3["fsm_state"] != "waiting_surname", (
                "Should advance from waiting_surname after providing surname"
            )
...
class TestNewClientRegistration:
    ...
    def test_new_client_registration(self):
        ...
        assert assert_response_contains(
            t2["response"], "telefono", "numero", "email", "servizio", "quale"
...
class TestFAQ:
    ...
    def test_faq_price_question(self):
        ...
        assert (
            "euro" in response_lower
            or "€" in response_lower
            or "costo" in response_lower
            or "prezzo" in response_lower
            or any(str(i) in response_lower for i in range(10, 100))
    ...
    def test_faq_hours_question(self):
        ...
        assert (
            "orario" in response_lower
            or "apertura" in response_lower
            or "chiusura" in response_lower
            or ":" in response_lower
            or any(str(i) + ":00" in response_lower for i in range(6, 22))
...
class TestCancel:
    ...
    def test_cancel_mid_booking(self):
        ...
        # Start booking
        ctx.turn("Vorrei prenotare")
        ctx.turn("Marco Rossi")
        ctx.turn("Un taglio")
        ctx.turn("Domani")
...
class TestEdgeCases:
    ...
    def test_empty_string(self):
        ...
        t1 = ctx.turn("")
        # Should not crash
        assert t1.get("_connection_failed") is not True, (
            "Empty input caused connection failure"
        )
        if t1["success"]:
    ...
    def test_very_long_string(self):
        ...
        # Should not crash
        assert t1.get("_connection_failed") is not True, (
            "Long input caused connection failure"
        )
        if t1["success"]:
            assert t1["response"] is not None
    ...
    def test_only_punctuation(self):
        ...
        # Should not crash
        assert t1.get("_connection_failed") is not True, (
            "Punctuation input caused connection failure"
        )
        if t1["success"]:
    ...
    def test_phone_number_input(self):
        ...
        assert t1.get("_connection_failed") is not True, (
            "Phone number caused connection failure"
        )
        if t1["success"]:
            assert t1["response"] is not None
    ...
    def test_special_characters(self):
        ...
        # Should not crash
        assert t1.get("_connection_failed") is not True, (
            "Special chars caused connection failure"
        )
        if t1["success"]:
            assert t1["response"] is not None
    ...
    def test_mixed_language(self):
        ...
        assert t1.get("_connection_failed") is not True, (
            "English caused connection failure"
        )
...
class TestLatency:
    ...
    def test_response_latency_acceptable(self):
        ...
        assert t1["_latency_ms"] < 2000, f"Response too slow: {t1['_latency_ms']:.0f}ms"
...
def test_health_check():
    ...
    assert h.get("status") in ("ok", "ready"), f"Pipeline health check failed: {h}"
