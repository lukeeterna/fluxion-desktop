...
class Results:
    ...
    def summary(self):
        ...
        print("\n" + "=" * 70)
        print(
            "RESULTS: %d OK / %d WARN / %d FAIL / %d SKIP (total %d)"
...
def test_booking_flow():
    ...
    steps = [
        (
            ...
            None,
            None,
        ),  # May go to idle or waiting_name/service
        ("Vorrei un taglio uomo", "waiting_name", ["nome", "cortesia"]),
        (
            "Mi chiamo Marco Rossi",
            None,
            None,
        ),  # May go to waiting_date, disambiguating, or waiting_service
    ...
    for text, exp_fsm, exp_words in steps:
        ...
        if exp_fsm and fsm == exp_fsm:
            ...
            R.WARN(
                "BOOKING",
                "%s -> fsm=%s (expected %s) but keywords OK (%.0fms)"
                % (text[:40], fsm, exp_fsm, ms),
            )
            ...
            R.FAIL(
                "BOOKING",
                "%s -> fsm=%s (expected %s) (%.0fms)" % (text[:40], fsm, exp_fsm, ms),
            )
    ...
    if (
        ...
        or "numero" in resp
        or "registr" in resp
        or fsm in ("registering_phone", "registering_surname", "propose_registration")
    ):
    ...
    if (
        "solito" in resp
        or "ultima volta" in resp
        or fsm in ("waiting_date", "waiting_service")
    ):
...
VERTICAL_TESTS = {
    "salone": {
        "services": [
            ...
            (
                "Vorrei un trattamento alla cheratina",
                ["trattamento", "cheratina", "waiting_name"],
            ),
    ...
    "professionale": {
        "services": [
            (
                ...
                ["dichiarazion", "redditi", "waiting_name"],
            ),
            (
                "Ho bisogno di una consulenza legale",
                ["consulenza", "legal", "avvocat", "waiting_name"],
            ),
...
def test_verticals():
    ...
    for vert_name, tests in VERTICAL_TESTS.items():
        ...
        for text, expected in tests.get("services", []):
            ...
            if matched:
                R.OK(
                    vert_name.upper(), "'%s' -> fsm=%s (%.0fms)" % (text[:45], fsm, ms)
                )
            else:
                R.WARN(
                    vert_name.upper(),
                    "'%s' -> fsm=%s, resp='%s' (%.0fms)"
                    % (text[:35], fsm, resp[:60], ms),
                )
        ...
        for text, expected in tests.get("faq", []):
            ...
            if matched:
                R.OK(
                    vert_name.upper() + "_FAQ",
                    "'%s' -> answered (%.0fms)" % (text[:40], ms),
                )
            else:
                R.WARN(
                    vert_name.upper() + "_FAQ",
                    "'%s' -> '%s' (%.0fms)" % (text[:35], resp[:60], ms),
                )
        ...
        for text, expected in tests.get("guardrail_block", []):
            ...
            if is_blocked:
                R.OK(
                    vert_name.upper() + "_GUARD",
                    "Blocked: '%s' (%.0fms)" % (text[:40], ms),
                )
            else:
                R.WARN(
                    vert_name.upper() + "_GUARD",
                    "NOT blocked: '%s' -> '%s' (%.0fms)" % (text[:30], resp[:60], ms),
                )
...
def test_disambiguation():
    ...
    fsm = r.get("fsm_state", "")
    variants_listed = sum(
        1 for v in ["taglio donna", "taglio uomo", "taglio bambino"] if v in resp
    )
    if variants_listed >= 3:
        R.FAIL(
            "FAMILY_DISAMB",
            "Listed %d taglio variants (should max 3 services)" % variants_listed,
        )
    ...
    if fsm in ("disambiguating_name", "disambiguating_birth_date"):
        ...
        R.WARN(
            "NAME_DISAMB",
            "No disambiguation (maybe no matching clients) -> fsm=%s" % fsm,
        )
...
def test_multi_service_edge_cases():
    ...
    for phrase, label in [
        ...
        resp = r.get("response", "").lower()
        if (
            fsm in ("confirming", "waiting_time")
            or "conferma" in resp
            or "disponibil" in resp
        ):
...
def test_vad():
    ...
    r = api(
        "/api/voice/vad/chunk",
        {"session_id": sid, "audio_hex": silence_pcm.hex(), "sample_rate": 16000},
    )
    if r.get("success"):
    ...
    r = api(
        "/api/voice/vad/chunk",
        {"session_id": sid, "audio_hex": speech_pcm.hex(), "sample_rate": 16000},
    )
    if r.get("success"):
        prob = r.get("probability", -1)
        state = r.get("state", "")
        ...
        R.OK(
            "VAD_SPEECH",
            "Speech: prob=%.2f, state=%s, event=%s" % (prob, state, event or "none"),
        )
    else:
    ...
    if r.get("success"):
        ...
        if vad_info.get("skipped") or not vad_info.get("has_speech"):
            ...
            R.WARN(
                "VAD_PROC_SIL",
                "Silence not detected as silence: %s" % str(vad_info)[:80],
            )
    else:
...
def test_timing():
    ...
    targets = [
        ...
        (
            ...
            "Vorrei taglio uomo barba e colore domani alle dieci",
            5000,
        ),
    ]
