...
class TestClientMatchingSecurity:
    ...
    def test_no_false_positive_name_only(self, state_machine):
        ...
        # Assert
        assert not result[0], "Non deve chiedere disambiguazione"
    ...
    def test_exact_match_name_surname(self, state_machine):
        ...
        state_machine.db_lookup = Mock(
            return_value=[{"id": "cust_001", "nome": "Gigio", "cognome": "Peruzzi"}]
        )
    ...
    def test_phonetic_disambiguation_gino_gigio(self, state_machine):
        ...
        # Assert
        assert result[0], "Deve chiedere disambiguazione"
    ...
    def test_phonetic_disambiguation_mario_maria(self, state_machine):
        ...
        assert result[0], "Deve chiedere disambiguazione per Mario/Maria"
...
class TestStateMachineTransitions:
    ...
    def test_all_states_defined(self, state_machine):
        ...
        assert len(all_states) == 15, (
            f"Expected 15 states, got {len(all_states)}: {[s.value for s in all_states]}"
        )
    ...
    def test_transition_idle_to_waiting_name(self, state_machine):
        ...
        state_machine.handle_input("Vorrei prenotare")
...
class TestErrorRecovery:
    ...
    def test_stt_low_confidence_recovery(self, state_machine):
        ...
        assert (
            "non ho capito" in result.response.lower()
            or "ripetere" in result.response.lower()
        )
    ...
    def test_api_failure_recovery(self, state_machine):
        ...
        assert (
            "problema tecnico" in result.response.lower()
            or "riprovare" in result.response.lower()
        )
...
class TestWhatsAppIntegration:
    ...
    def test_whatsapp_message_format(self):
        ...
        # Test formattazione
        formatted = template.format(nome="Mario", data="15/03/2026", ora="15:00")
...
class TestPerformance:
    ...
    def test_memory_usage(self):
        ...
        assert memory_per_session < 5_000_000, (
            f"Memory per session: {memory_per_session} bytes"
        )
...
class TestAPIEndpoints:
    ...
    async def test_health_endpoint(self):
        ...
        health_response = {"status": "ok", "version": "1.0.0", "timestamp": time.time()}
...
class TestE2EScenarios:
    ...
    def test_e2e_new_customer_booking(self, state_machine):
        ...
        state_machine.handle_input("Mi chiamo Mario Rossi")
