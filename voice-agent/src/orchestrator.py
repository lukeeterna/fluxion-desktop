    ...
    async def process(
        ...
        if response is None:
            ...
            if _is_standalone_goodbye:
                should_exit = True
                # B3-FIX1: mark congedo UNCONDITIONALLY (was nested in `if not response`
                # → skipped when frustration/cortesia had already populated `response`,
                # leaving intent='' so the VoIP guard suppressed the legit S142 hangup).
...
{self._get_time_pressure_note()}"""
    ...
    def _search_client_sqlite_fallback(self, name: str) -> Dict[str, Any]:
        ...
        query = (
            ...
            "FROM clienti WHERE deleted_at IS NULL AND ("  # nosec B608
            + where_clause
            + ") ORDER BY cognome ASC, nome ASC LIMIT 10"
        )
