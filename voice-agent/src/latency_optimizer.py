...
class FluxionConnectionPool:
    ...
    async def initialize(self):
        ...
        logger.info(
            f"[FluxionConnectionPool] Initialized: max_conn={self.max_connections}"
        )
    ...
    async def close(self):
        ...
        logger.info(
            f"[FluxionConnectionPool] Closed. Requests: {self._requests_count}, Reused: {self._connections_reused}"
        )
...
class FluxionStreamingLLM:
    ...
    async def stream_response(
        ...
        if len(prompt) > 4000:  # ~2000 tokens
            ...
            logger.warning("[FluxionStreamingLLM] Prompt troncato a 3500 chars")
...
async def test_latency_optimizer():
    ...
    async for chunk in opt.optimize_llm_call(
        "Saluta il cliente e chiedi che servizio desidera", context, complexity="simple"
    ):
