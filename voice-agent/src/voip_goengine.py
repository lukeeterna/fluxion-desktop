...
class GoEngineVoIPManager:
    ...
    def _audio_processing_loop(self):
        ...
        # FIX-BARGEIN-2: finestra scorrevole dell'RMS TX di Sara (~20 frame ≈400ms).
        # tx_ref = max(window) → soglia barge riferita al TX, robusta al ritardo TX→eco.
        tx_rms_window: "deque[float]" = deque(maxlen=20)
        # Marker diagnostico [RX-MARK] rate-limited ~1/s.
        last_marker_ts = 0.0
        ...
        while self._running and not self._stop_evt.is_set():
            ...
            # FIX-BARGEIN-2: soglia barge riferita al TX di Sara (floor RX fuori formula).
            # L'eco di linea è ∝ energia TX → thr = k·tx_ref rigetta l'eco senza rincorrere
            # un floor RX che, sotto eco continuo, resta basso e lascia passare i picchi.
            tx_rms_window.append(self._current_tx_rms)
            tx_ref = max(tx_rms_window) if tx_rms_window else 0.0
            thr = max(BARGE_IN_MARGIN, BARGE_TX_K * tx_ref)

            # Marker diagnostico rate-limited ~1/s (rms/tx_ref/thr/floor/stato/sustain).
            now = time.monotonic()
            if now - last_marker_ts >= 1.0:
                last_marker_ts = now
                logger.info(
