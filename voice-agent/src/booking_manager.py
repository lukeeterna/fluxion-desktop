...
class BookingManager:
        ...
        if not available:
            ...
            return (
                ...
                None,
                {
                    ...
                    "can_waitlist": True,
                    "message": self._build_alternatives_message(
    ...
    def cancel_booking(
        ...
        self._handle_slot_freed(
            booking.business_id, booking.service_id, booking.date, booking.time
        )
    ...
    def reschedule_booking(
        ...
        self._handle_slot_freed(
            booking.business_id, booking.service_id, old_date, old_time
        )
    ...
    def check_waitlist_priority(
        self, service_id: str, date: str, time: str
    ) -> Optional[str]:
        """
    ...
    def get_available_slots(
        ...
        # Calcola slot liberi
        slots = self._calculate_available_slots(business_hours, existing, duration)

        return slots

    def get_preferred_operator_availability(
        self, operator_id: str, date: str
    ) -> List[str]:
    ...
    def _get_service_info(self, service_id: str) -> Dict:
        """Recupera info servizio"""
        return self.db.get_service(service_id) or {"name": "Servizio", "duration": 30}
    ...
    def _check_availability(
        ...
        for booking in existing:
            ...
            try:
                ...
                old_duration = (
                    booking.duration_minutes if booking.duration_minutes else 60
                )
    ...
    def _find_alternative_slots(
        ...
        same_day_slots = self.get_available_slots(
            business_id, service_id, preferred_date, operator_id
        )
        ...
        earlier_slots = [
            s for s in same_day_slots if datetime.strptime(s, "%H:%M") < preferred_dt
        ]
        ...
        for i in range(max(len(earlier_slots), len(later_slots))):
            ...
            if i < len(later_slots) and len(alternatives) < max_alternatives:
                alternatives.append(
                    {"date": preferred_date, "time": later_slots[i], "type": "later"}
                )
        ...
        if len(alternatives) < max_alternatives:
            for day_offset in range(1, 4):  # Prossimi 3 giorni
                ...
                next_slots = self.get_available_slots(
                    business_id, service_id, next_date, operator_id
                )
    ...
    def _build_alternatives_message(
        self, alternatives: List[Dict], requested_date: str, requested_time: str
    ) -> str:
        """
        ...
        for i, alt in enumerate(alternatives[:3], 1):
            ...
            if alt["type"] == "earlier":
                ...
                alt_texts.append(
                    f"{i}) {day_names_it.get(day_name, day_name)} {date_str} alle {alt['time']}"
                )
    ...
    def _handle_slot_freed(
        self, business_id: str, service_id: str, date: str, time: str
    ):
        """
        ...
        waitlist_entries = self.waitlist.find_entries_for_slot(
            service_id=service_id, date=date, time=time, business_id=business_id
        )
        ...
        waitlist_entries.sort(
            key=lambda e: (
                0
                if e.customer_tier == CustomerTier.PLATINUM
                else 1
                if e.customer_tier == CustomerTier.GOLD
                else 2
                if e.customer_tier == CustomerTier.SILVER
                else 3,
                e.created_at,
            )
        )
        ...
        if customer and self.whatsapp:
            ...
            self._send_waitlist_notification(
                customer, service_id, date, time, top_entry.entry_id
            )

            # Marca come "notificato" in attesa di risposta
            self.waitlist.mark_as_notified(
                entry_id=top_entry.entry_id, slot_date=date, slot_time=time
            )
    ...
    def _send_reschedule_notice(
        self, booking: Booking, customer: CustomerProfile, old_date: str, old_time: str
    ):
