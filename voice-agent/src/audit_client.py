...
class AuditClient:
    ...
    def _get_connection(self) -> sqlite3.Connection:
        ...
        if not hasattr(self._local, "connection") or self._local.connection is None:
            ...
            try:
                cols = [
                    r[1]
                    for r in self._local.connection.execute(
        ...
        try:
            ...
            if action == AuditAction.UPDATE and data_before and data_after:
                changed_fields = json.dumps(
                    self._calculate_diff(data_before, data_after)
                )
            ...
            """,
                (
                    ...
                    entity_type,
                    entity_id,
                    json.dumps(data_before, ensure_ascii=False)
                    if data_before
                    else None,
            ...
            logger.error(
                "[AUDIT] Unexpected error logging operation: %s", e, exc_info=True
            )
            return None
    ...
    def _get_retention_days(self, category: GdprCategory) -> int:
        ...
        try:
            ...
            logger.debug(
                f"[AUDIT] Could not load retention from DB, using defaults: {e}"
            )
        except Exception as e:
            logger.debug(
                "[AUDIT] Unexpected error loading retention: %s", e, exc_info=True
            )
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if isinstance(result, dict):
                    entity_id = (
                        result.get("id")
                        or result.get("cliente_id")
                        or result.get("appuntamento_id")
                    )
                    if entity_id:
