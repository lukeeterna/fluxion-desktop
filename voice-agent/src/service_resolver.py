...
class FuzzyMatcher:
    ...
    @staticmethod
    def find_best_match(
        query: str, candidates: List[Dict], threshold: float = 0.6
        ...
        for candidate in candidates:
            ...
            name_score = SequenceMatcher(
                None, query_lower, candidate["name"].lower()
            ).ratio()
...
class ServiceResolver:
    ...
    def resolve(
        self, extracted_text: str, business_id: str, category_hint: Optional[str] = None
        ...
        # Fuzzy match
        match, score = self.matcher.find_best_match(
            extracted_text, services, threshold=0.6
        )
    ...
    def get_suggestions(
        self, partial_text: str, business_id: str, limit: int = 3
    ) -> List[Dict]:
        ...
        for service in services:
            score = SequenceMatcher(
                None, partial_text.lower(), service["name"].lower()
            ).ratio()
...
class OperatorResolver:
    ...
    def resolve(
        ...
        match, score = self.matcher.find_best_match(
            extracted_name, operators, threshold=0.7
        )
    ...
    def get_available_operators(
        self, business_id: str, date: str, time: str, service_id: Optional[str] = None
    ) -> List[Dict]:
    ...
    def get_preferred_or_available(
        ...
        if preferred_id:
            ...
            if self._check_operator_availability(preferred_id, date, time):
                ...
                preferred = {"id": op["id"], "name": op["name"], "is_preferred": True}

        # Recupera tutti gli altri disponibili
        all_available = self.get_available_operators(
            business_id, date, time, service_id
        )
    ...
    def _get_operators_for_business(
        self, business_id: str, service_id: Optional[str] = None
    ) -> List[Dict]:
    ...
    def _check_operator_availability(
        self, operator_id: str, date: str, time: str
    ) -> bool:
        ...
        if service_text:
            ...
            if not success:
                ...
                result["suggestions"]["services"] = (
                    self.service_resolver.get_suggestions(service_text, business_id)
                )
