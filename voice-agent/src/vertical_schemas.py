...
class SchedaFisioterapia:
    ...
    # Esiti
    esito_trattamento: Optional[str] = (
        None  # "miglioramento", "stabile", "peggioramento"
    )
...
class SchedaVeicolo:
    ...
    alimentazione: Optional[str] = (
        None  # "benzina", "diesel", "gpl", "metano", "elettrico"
    )
...
class WaitlistEntry:
    ...
    def priority_score(self) -> int:
        ...
        urgency_scores = {"urgent": 200, "high": 100, "normal": 0, "low": -50}
    ...
    def get_medical_warnings(self) -> List[str]:
        ...
        for vertical, card in self.cards.items():
            if hasattr(card, "patologie_croniche") and card.patologie_croniche:
                warnings.append(
                    f"[{vertical}] Patologie: {', '.join(card.patologie_croniche)}"
                )
            if hasattr(card, "terapie_farmacologiche") and card.terapie_farmacologiche:
                warnings.append(
                    f"[{vertical}] Terapie in corso: {len(card.terapie_farmacologiche)} farmaci"
                )
            ...
            if hasattr(card, "patologie_attive") and card.patologie_attive:
                warnings.append(
                    f"[{vertical}] Patologie attive: {', '.join(card.patologie_attive)}"
                )
        return warnings
...
class CustomerCardFactory:
    ...
    def create_card(vertical: str, customer_id: str) -> Any:
        ...
        if card_type == "fisioterapia":
            ...
            return SchedaCarrozzeria(
                customer_id=customer_id, veicolo_id="", created_at=now
            )
        else:
