...
class TimeConstraint:
    ...
    def matches(self, candidate: time) -> bool:
        ...
        if self.constraint_type == TimeConstraintType.EXACT:
            ...
            TimeConstraintType.FIRST_AVAILABLE,
        ):
            return True
        return True
...
class ExtractedTime:
    ...
    time_constraint: Optional["TimeConstraint"] = (
        None  # World-class: TIMEX3 semantic preservation
    )
    ...
    def get_display(self) -> str:
        ...
        if (
            ...
            and self.time_constraint.constraint_type != TimeConstraintType.EXACT
        ):
...
def extract_date(
    ...
    for day_name, weekday in DAYS_IT.items():
        if day_name in text_lower:
            ...
            # If weekday < today_weekday, modulo already wrapped to next week
                ...
                "settimana prossima" in text_lower or "prossima settimana" in text_lower
            ):
    ...
    fra_word_pattern = (
        rf"(?:fra|tra)\s+({it_num_pattern})\s+(giorn[oi]|settiman[ae]|mes[ei])"
    )
    ...
    if re.search(
        r"\b(?:il\s+)?mes[ei]\s+prossim[oa]\b|\bprossim[oa]\s+mes[ei]\b", text_lower
    ):
    ...
    if HAS_DATEPARSER:
        try:
            ...
            if parsed:
                return ExtractedDate(
                    date=parsed, original_text=text, confidence=0.7, is_relative=False
                )
        except Exception:
            pass
...
def extract_time(text: str) -> Optional[ExtractedTime]:
    ...
    m = re.search(
        r"(?:verso\s+le|intorno\s+alle?|circa\s+alle?|sulle)\s+(\d{1,2})\b", text_n
    )
    if m:
        h = int(m.group(1))
        if 0 <= h <= 23:
...
def extract_time_constraint(text: str) -> Optional[TimeConstraint]:
    ...
    text_lower = text.lower()
    if re.search(
        r"\bprima\s+possibile\b|\bsubito\b|\bquando\s+siete\s+liberi\b", text_lower
    ):
...
def extract_name(text: str) -> Optional[ExtractedName]:
    ...
    patterns = [
        ...
        (
            r"(?:parlo\s+per|a\s+nome\s+di)\s+([A-Z][a-zàèéìòù]+(?:\s+[A-Z][a-zàèéìòù]+)?)",
            0.85,
        ),
        # "prenotazione per Mario Rossi" / "appuntamento per"
        (
            r"(?:prenotazione|appuntamento|prenota|prenotare)\s+per\s+([A-Z][a-zàèéìòù]+(?:\s+[A-Z][a-zàèéìòù]+)?)",
            0.9,
        ),
        ...
        # "il mio nome è Giovanni"
        (
            r"(?:il\s+)?mio\s+nome\s+(?:è|e)\s+([A-Z][a-zàèéìòù]+(?:\s+[A-Z][a-zàèéìòù]+)?)",
            0.95,
        ),
        # "mi chiamo Laura"
        ...
        (
            r"(?:sono|io\s+sono)\s+(?:(?:il|la|lo)\s+)?([A-Z][a-zàèéìòù]+(?:\s+[A-Z][a-zàèéìòù]+){0,2})",
            0.95,
        ),
    ]
    ...
    for pattern, confidence in patterns:
        ...
        if match:
            ...
            return ExtractedName(
                name=name, original_text=match.group(0), confidence=confidence
            )
...
def extract_operator(text: str) -> Optional[ExtractedOperator]:
    ...
    patterns = [
        ...
        (
            r"con\s+l['\u2019\s]?\s*(?:operatrice|operatore)\s+([A-Z][a-zàèéìòù]+(?:\s+[A-Z][a-zàèéìòù]+)?)",
            0.95,
        ),
        ...
        # "vorrei/preferisco X"
        (
            r"(?:vorrei|preferisco|preferirei)\s+([A-Z][a-zàèéìòù]+(?:\s+[A-Z][a-zàèéìòù]+)?)",
            0.75,
        ),
    ]
...
def extract_operators_multi(text: str) -> Optional[ExtractedOperatorList]:
    ...
    trigger_matched = any(re.search(t, text, re.IGNORECASE) for t in _MULTI_OP_TRIGGERS)
...
def extract_service(
    text: str, services_config: Dict[str, List[str]], fuzzy_threshold: float = 0.8
...
_REFERENTIAL_PATTERNS_COMPILED = [
    ...
    re.compile(
        r"(?:quell[oiae]\s+(?:di|che)\s+(?:prima|detto|chiesto))", re.IGNORECASE
    ),
    re.compile(
...
def extract_services(
    ...
    if len(_matches_with_specificity) > 1:
        ...
        _matches_with_specificity = [
            m for m in _matches_with_specificity if m[2] >= max_len * 0.7
        ]
        ...
        if len(_matches_with_specificity) > 1:
            ...
            if _exact_name_matches:
                ...
                min_words = min(
                    len(re.split(r"[\s/]+", services_config.get(m[0], [""])[0]))
                    for m in _matches_with_specificity
                )
                _matches_with_specificity = [
                    m
                    for m in _matches_with_specificity
                    if len(re.split(r"[\s/]+", services_config.get(m[0], [""])[0]))
                    == min_words
                ]
    ...
    if len(_matches_with_specificity) > 3:
        ...
        _ambiguous_families = {
            base: members
            for base, members in _family_groups.items()
            if len(members) >= 3
        }
...
def extract_phone(text: str) -> Optional[str]:
    ...
    _has_digit_words = (
        len(_text_words & _ITALIAN_DIGIT_WORDS) >= 3
    )  # at least 3 digit words
...
class ExtractionResult:
    ...
    operators: List[ExtractedOperator] = field(
        default_factory=list
    )  # Multi-op list (GAP-P1-8)
...
def extract_all(
    ...
    if services_results:
        ...
        if all(conf < 0 for _, conf in services_results) and len(services_results) > 1:
            ...
            result.service = services_results[0][
                0
            ]  # First service for backwards compat
...
_MEDICAL_URGENCY_PATTERNS = [
    (
        re.compile(
            r"\b(?:urgente|urgenza|urgentissimo|prima\s+possibile|subito|oggi\s+stesso|immediatamente)\b",
            re.IGNORECASE,
        ),
        "urgente",
    ),
    ...
    (
        re.compile(
            r"\b(?:dolore\s+(?:forte|acuto|intenso)|forte\s+(?:dolore|mal\s+di)|mal\s+di\s+(?:denti|testa|schiena|stomaco|pancia))\b",
            re.IGNORECASE,
        ),
        "urgente",
    ),
    (
        re.compile(
            r"\b(?:fatico\s+a\s+respirare|non\s+riesco\s+a\s+respirare|dolore\s+al\s+petto|dolore\s+toracico|fiato\s+corto)\b",
            re.IGNORECASE,
        ),
        "urgente",
    ),
    (
        re.compile(
            r"\b(?:sangue|emorragia|svenimento|perdita\s+di\s+coscienza|febbre\s+alta|gonfiore\s+(?:forte|grave))\b",
            re.IGNORECASE,
        ),
        "urgente",
    ),
    (
        re.compile(
            r"\b(?:presto|appena\s+possibile|quanto\s+prima|questa\s+settimana)\b",
            re.IGNORECASE,
        ),
        "alta",
    ),
    (
        re.compile(
            r"\b(?:quando\s+c[\u2019'][\xe8e]\s+posto|non\s+ho\s+fretta|con\s+calma|quando\s+volete)\b",
            re.IGNORECASE,
        ),
        "bassa",
    ),
...
_MEDICAL_VISIT_TYPES = {
    "prima_visita": [
        ...
        r"prima\s+volta",
        r"nuovo\s+paziente",
        r"non\s+sono\s+mai\s+venuto",
    ],
    "controllo": [
        r"controllo",
        r"follow.up",
        r"follow\s+up",
        r"visita\s+di\s+controllo",
        r"ricontrollo",
    ],
...
class VerticalEntities:
    ...
    sub_vertical: Optional[str] = (
        None  # e.g. "barbiere", "nail_specialist", "estetista_viso"
    )
...
_NEGATIVE_DAY_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (
        re.compile(
            r"\b(?:non|tranne|eccetto|escluso?|se\s+non\s+è\s+(?:il|lo|la|un)?\s*)(?:\s+il|\s+di|\s+ogni)?\s*luned[iì]\b",
            re.IGNORECASE,
        ),
        "monday",
    ),
    (
        re.compile(
            r"\b(?:non|tranne|eccetto|escluso?)(?:\s+il|\s+di|\s+ogni)?\s*marted[iì]\b",
            re.IGNORECASE,
        ),
        "tuesday",
    ),
    (
        re.compile(
            r"\b(?:non|tranne|eccetto|escluso?)(?:\s+il|\s+di|\s+ogni)?\s*mercoled[iì]\b",
            re.IGNORECASE,
        ),
        "wednesday",
    ),
    (
        re.compile(
            r"\b(?:non|tranne|eccetto|escluso?)(?:\s+il|\s+di|\s+ogni)?\s*gioved[iì]\b",
            re.IGNORECASE,
        ),
        "thursday",
    ),
    (
        re.compile(
            r"\b(?:non|tranne|eccetto|escluso?)(?:\s+il|\s+di|\s+ogni)?\s*venerd[iì]\b",
            re.IGNORECASE,
        ),
        "friday",
    ),
    (
        re.compile(
            r"\b(?:non|tranne|eccetto|escluso?)(?:\s+il|\s+di|\s+ogni)?\s*sabato\b",
            re.IGNORECASE,
        ),
        "saturday",
    ),
    (
        re.compile(
            r"\b(?:non|tranne|eccetto|escluso?)(?:\s+il|\s+di|\s+ogni)?\s*domenica\b",
            re.IGNORECASE,
        ),
        "sunday",
    ),
