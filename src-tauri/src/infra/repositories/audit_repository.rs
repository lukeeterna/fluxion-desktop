...
impl SqliteAuditLogRepository {
    ...
    fn build_query(&self, base_query: &str, query: &AuditLogQuery) -> (String, Vec<sqlx::sqlite::SqliteArgumentValue<'_>>) {
        let mut conditions = Vec::new();
        let mut params: Vec<sqlx::sqlite::SqliteArgumentValue<'_>> = Vec::new();
        ...
        if let Some(user_type) = query.user_type {
            ...
            params.push(Self::user_type_to_db(user_type).into());
        }

        if let Some(ref action) = query.action {
            conditions.push("action = ?".to_string());
            params.push(format!("{:?}", action).to_uppercase().into());
        }
        ...
        if let Some(ref entity_id) = query.entity_id {
            ...
            params.push(entity_id.clone().into());
        }

        if let Some(source) = query.source {
            conditions.push("source = ?".to_string());
            params.push(Self::source_to_db(source).into());
        }

        if let Some(category) = query.gdpr_category {
            conditions.push("gdpr_category = ?".to_string());
            params.push(Self::gdpr_category_to_db(category).into());
        }

        if let Some(ref from_date) = query.from_date {
            conditions.push("timestamp >= ?".to_string());
            params.push(from_date.to_rfc3339().into());
        }

        if let Some(ref to_date) = query.to_date {
            conditions.push("timestamp <= ?".to_string());
            params.push(to_date.to_rfc3339().into());
        }
        ...
        let sql = base_query.replace("{where}", &where_clause);
        (sql, params)
    }
}
...
impl AuditLogRepository for SqliteAuditLogRepository {
    ...
    async fn query(&self, query: &AuditLogQuery) -> Result<Vec<AuditLog>, RepositoryError> {
        ...
        let (sql, _params) = self.build_query(base_query, query);
        
        // For now, use a simpler approach with direct parameter binding
        ...
        if let Some(user_type) = query.user_type {
            db_query = db_query.bind(Self::user_type_to_db(user_type));
        }
        if let Some(ref action) = query.action {
            db_query = db_query.bind(format!("{:?}", action).to_uppercase());
        }
        if let Some(ref entity_type) = query.entity_type {
            db_query = db_query.bind(entity_type);
        }
        if let Some(ref entity_id) = query.entity_id {
            db_query = db_query.bind(entity_id);
        }
        if let Some(source) = query.source {
            db_query = db_query.bind(Self::source_to_db(source));
        }
        if let Some(category) = query.gdpr_category {
            db_query = db_query.bind(Self::gdpr_category_to_db(category));
        }
        if let Some(ref from_date) = query.from_date {
            db_query = db_query.bind(from_date.to_rfc3339());
        }
        if let Some(ref to_date) = query.to_date {
            db_query = db_query.bind(to_date.to_rfc3339());
        }
    ...
    async fn count(&self, query: &AuditLogQuery) -> Result<i64, RepositoryError> {
        let base_query = "SELECT COUNT(*) as count FROM audit_log {where}";
        let (sql, _) = self.build_query(base_query, query);
