...
pub async fn update_appuntamento(
    ...
    .bind(
        input
            .cliente_id
            .unwrap_or_else(|| current.cliente_id.clone()),
    )
    .bind(
        input
            .servizio_id
            .unwrap_or_else(|| current.servizio_id.clone()),
    )
    .bind(new_operatore)
    .bind(new_start)
    .bind(&new_end)
    ...
    .bind(input.note.or_else(|| current.note.clone()))
    .bind(input.note_interne.or_else(|| current.note_interne.clone()))
