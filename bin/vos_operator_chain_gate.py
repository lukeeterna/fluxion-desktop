...
def validate(root: Path) -> dict:
    ...
    if (
        ...
        or data["contract_id"] != "FLUXION-OPERATOR-CHAIN-V1"
    ):
    ...
    exact_capabilities = {
        ...
        "FOUNDER": {
            "required": {
                "AUTHORIZE_CONFIRM_FIRST",
                "AUTHORIZE_IRREVERSIBLE",
            },
            "forbidden": {
                "ORDINARY_MACHINE_EXECUTION",
                "ORDINARY_COPY_PASTE",
            },
        },
    }
    ...
    node = (root / "docs/judge/CC-WEB-NODE-PROMPT.md").read_text(
        encoding="utf-8"
    )
    review = (
        root / "docs/judge/CLAUDE-WEB-SONNET-REVIEW-CONTRACT.md"
    ).read_text(encoding="utf-8")
    node_lower = node.lower()
    for phrase in (
        "non revisionare semanticamente",
        "<!-- fluxion-ccweb-node -->",
        "ready_for_claude_web_review",
    ):
        if phrase.lower() not in node_lower:
            raise GateError(f"CC Web node contract missing: {phrase}")
    ...
    forbidden_present = [
        str(path)
        for path in FORBIDDEN_REVIEW_AUTHORITY
        if (root / path).exists()
    ]
    return {
        "schema_version": 1,
        ...
        "forbidden_reviewer_substitutions_present": forbidden_present,
...
def main() -> int:
    ...
    try:
        ...
        if args.json:
            print(
                json.dumps(
                    {"static_status": "BLOCKED", "error": str(exc)},
                    sort_keys=True,
                )
            )
        else:
